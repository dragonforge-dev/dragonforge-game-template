...
#var _current_focused_control: Control
#var _can_pause: bool = false;


func _ready() -> void:
	for screen in get_children():
		if screen is Screen and screen is not SplashScreen:
	...
	if bypass_splash_screens and OS.is_debug_build():
		...
		_current_screen = ScreenOld.Type.Splash
	...
	#get_viewport().connect("gui_focus_changed", _onGuiFocusChanged)


func _input(event: InputEvent) -> void:
	if event.is_action_pressed("back_button"):
		_onScreenCloseRequest()
	#if Controls.joystick_ui_input_cooldown_timer.is_stopped():
		#if event.is_action_pressed("left") and _current_focused_control is not HSlider:
				#_onScreenCloseRequest()
				#Controls.joystick_ui_input_cooldown_timer.start()
		#if event.is_action_pressed("right") and _current_focused_control is Button:
...
func _onScreenCloseRequest() -> void:
	var current_screen = _screens[_current_screen]
...
#func _onGuiFocusChanged(control: Control) -> void:
	#if control != null:
